...
function renderPeople(){
  ...
  const backlog=p=>{const value=p.backlog||{},tasks=Number(value.tasks)||0,tickets=Number(value.tickets)||0,total=Number(value.total)||(tasks+tickets);return `<span class="backlog-cell" title="Asignaciones abiertas al momento de la última actualización"><b>${total}</b><small>${tasks} proyecto${tasks===1?"":"s"} · ${tickets} ticket${tickets===1?"":"s"}</small></span>`};
