...
function renderTimeline(rows){
  ...
  $("#periodTimeline").innerHTML=Object.entries(groups).map(([period,items],index)=>`<details class="period" ${index===0?"open":""}><summary><span>${state.timelineView==="day"?dateLabel(period):new Intl.DateTimeFormat("es-AR",{month:"long",year:"numeric",timeZone:"UTC"}).format(new Date(`${period}-15T12:00:00Z`))}</span><small>${items.length} eventos</small></summary><div class="timeline">${items.map(a=>`<article class="event" style="--area:${AREA[a.area].color}"><time class="event-time">${a.time}</time><i class="event-dot"></i><div class="event-card"><div><div class="event-meta"><span class="pill">${AREA[a.area].label}</span><span class="source">${a.source}</span></div><h3>${a.title}</h3><p>${a.description}</p></div><span class="person">${a.person}</span></div></article>`).join("")}</div></details>`).join("");
}
